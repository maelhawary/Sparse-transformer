  ...
  #              dim,
   #             use_xpos = use_xpos,
    ...
    def forward(
       ...
       # print('shape_att',attn.shape)
       # print('shape_bv',bv.shape)
        # aggregation
...
class Multiheads(nn.Module):
    def __init__(self,number_of_heads,d_model,d_att_weigh,seq_len,dropout,window_size) -> None:
        super().__init__()
        ...
        self.atten_head=LocalAttention(window_size) ## add multiheads and add dropout so you can say that it is inspired by .. + some modifications
        #nn.ModuleList([LocalAttention(d_model,seq_len,self.head_size,dropout,window_size) for i in range (number_of_heads) ])#
        self.proj = nn.Linear(self.head_size * number_of_heads, d_att_weigh)
        self.dropout = nn.Dropout(dropout)

    def forward(self, x):
        #out = torch.cat([h(x) for h in self.Multiheads], dim=-1)
        out=self.atten_head(x,x,x)
